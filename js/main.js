...
let customScale = 1;
let targetFps = 60;
let bgmfile = './assets/audio/BlueNewWorld.ogg';
let bgmvolume = 0;
let bgm;
...
const HITBOX = {
	headpat: { xMin: 1450, xMax: 2070, yMin: 0, yMax: 360 },
	voiceline: { xMin: 1530, xMax: 2130, yMin: 830, yMax: 1600 }
};
...
function load() {
	...
	if (assetManager.isLoadingComplete() && typeof introAnimation !== 'undefined') {
		...
		if (introAnimation) {
			...
			if (mouseOptions.voicelines) {
				// 播放第一段介绍语音
				let track1 = new Audio(`./assets/audio/${CHARACTER}_memoriallobby_0_1.ogg`);
				...
				// 播放第二段介绍语音
				let track2 = new Audio(`./assets/audio/${CHARACTER}_memoriallobby_0_2.ogg`);
